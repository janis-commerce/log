...
module.exports = class Log {
	...
	static preFormatLog({ log, ...logObject }, client, privateFields) {
		...
		// A log must identify its target with the singular entity or with relatedEntities (entity:id
		// tokens). entity is no longer mandatory: a log can carry only relatedEntities (possibly multi-entity).
		if(!logObject.entity && !logObject.relatedEntities?.length)
			throw new LogError('A log requires entity or relatedEntities', LogError.codes.INVALID_LOG);

		// The distinct set of entities the log relates to: its own entity first, then any extra entity
		// prefixes carried by the relatedEntities tokens (`entity:id`). Individual writes end up with
		// [entity]; a multi-entity grouped write (e.g. relatedEntities ['base-price:2','price:6'] with
		// entity 'price') ends up with ['price','base-price']. Trace reads the entity filter off this set.
		logObject.entities = [...new Set(
			[logObject.entity, ...(logObject.relatedEntities || []).map(token => token.split(':')[0])].filter(Boolean)
		)];
